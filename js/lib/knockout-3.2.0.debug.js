...
(function () {
    ...
    function simpleHtmlParse(html) {
        ...
        return ko.utils.makeArray(div.lastChild.children);
    }
...
ko.memoization = (function () {
    ...
    function findMemoNodes(rootNode, appendToArray) {
        ...
        if (rootNode.nodeType == 8) {
            ...
            for (var i = 0, childNodes = rootNode.children, j = childNodes.length; i < j; i++)
...
(function() {
    ...
    ko.virtualElements = {
        ...
        childNodes: function(node) {
            return isStartComment(node) ? getVirtualChildren(node) : node.children;
        },
        ...
        setDomNodeChildren: function(node, childNodes) {
            if (!isStartComment(node))
                ...
                for (var i = 0, j = childNodes.length; i < j; i++)
                    endCommentNode.parentNode.insertBefore(children[i], endCommentNode);
            }
        },
...
(function(undefined) {
    ...
    function resolveTemplate(errorCallback, templateConfig, callback) {
        if (typeof templateConfig === 'string') {
            ...
            callback(ko.utils.makeArray(templateConfig.children));
        ...
        switch (ko.utils.tagNameLower(elemInstance)) {
            ...
            case 'template':
                ...
                if (isDocumentFragment(elemInstance.content)) {
                    return ko.utils.cloneNodes(elemInstance.content.children);
                }
        }

        // Regular elements such as <div>, and <template> elements on old browsers that don't really
        // understand <template> and just treat it as a regular container
        return ko.utils.cloneNodes(elemInstance.children);
    }
...
(function () {
    ...
    ko.utils.setDomNodeChildrenFromArrayMapping = function (domNode, array, mapping, options, callbackAfterAddingNodes) {
        ...
        for (var i = 0, nextNode = ko.virtualElements.firstChild(domNode), lastNode, node; mapData = itemsToProcess[i]; i++) {
            ...
            //console.time('mapNode');
            if (!mapData.mappedNodes)
                ko.utils.extend(mapData, mapNodeAndRefreshWhenChanged(domNode, mapping, mapData.arrayEntry, callbackAfterAddingNodes, mapData.indexObservable));
            //console.timeEnd('mapNode');
            ...
            //console.time('applyBinding');
            if (!mapData.initialized && callbackAfterAddingNodes) {
                callbackAfterAddingNodes(mapData.arrayEntry, mapData.mappedNodes, mapData.indexObservable);
                mapData.initialized = true;
            }
            //console.timeEnd('applyBinding');
            mapDatas.push(mapData);
        }

        //console.time('append');
        ko.virtualElements.insertAfter(domNode, toInsert, lastNode);
        //console.timeEnd('append');
...
ko.nativeTemplateEngine.prototype['renderTemplateSource'] = function (templateSource, bindingContext, options) {
    ...
    if (templateNodes) {
        ...
        if (templateNodes instanceof Array) {
            ...
            nodes = ko.utils.makeArray(templateNodes.cloneNode(true).children);
        }
        return nodes;
    } else {
